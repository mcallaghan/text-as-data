...
\CommentTok{\# We just need to create a dictionary to map feature names to column indices}
\NormalTok{vi }\OperatorTok{=} \BuiltInTok{dict}\NormalTok{(}\BuiltInTok{zip}\NormalTok{(vocab, }\BuiltInTok{range}\NormalTok{(dfm.shape[}\DecValTok{1}\NormalTok{])))}
