...
X = dfm.toarray()
# We just need to create a dictionary to map feature names to column indices
vi = dict(zip(vocab, range(dfm.shape[1])))
...
On each round, the spammer can add 4 texts, and the spammer can add 1 coefficient (and edit the others as well as any pre-processing steps).
