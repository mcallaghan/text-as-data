...
- `[G-Z]` matches any upper case letter from G to Z

## Exercise

Load the dataset of papers which mention net zero. Use regular expressions to extract dates in the future from their abstracts. How many papers mention a date? If we extract the first date using `str_extract()`, what dates are most common?

## Downloading papers in R

\tiny
```{r echo=TRUE, include=TRUE, error=TRUE, message=FALSE}
library(jsonlite)
library(dotenv)
library(httr)
library(dplyr)
...
Next week we'll be brushing up on our ggplot / matplotlib / seaborn skills, and putting them to use in plotting text data
